...
const UserProfilePage = () => {
  ...
  async function onSaveRecentSearch() {
    ...
    try {
      ...
      setSaveMessage(e?.message || 'Failed to store recent search (authentication required).')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <main className="px-4 md:px-16 lg:px-24 xl:px-32 py-8">
      ...
      <p className="text-sm text-gray-600 mt-1">User role and recent searched cities.</p>
      ...
      {!loading && (
        <div className="mt-6 grid gap-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-4">
            ...
            {saveMessage && <div className="mt-3 text-sm text-gray-700">{saveMessage}</div>}
          </div>
        </div>
      )}

      <div className="mt-6 text-xs text-gray-500">
        Note: /api/user and /api/user/store-recent-search are protected.
      </div>
    </main>
  )
}
